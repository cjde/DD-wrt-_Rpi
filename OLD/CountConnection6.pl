	...
	$timestamp_old = $timestamp;
		...
		foreach my $srcip ( sort keys %sessions )  {
			...
			foreach my $desip (sort keys %hoh ) {
				...
				print " $srcip connected to $desip / $desname getting deltas\n";
				...
				print  OUTFILE "	Reply $desname \t Total  ",$sessions{ $srcip }{ $desip }{o_T_bytes}, " / ", $sessions{ $srcip }{ $desip }{r_T_bytes};
				...
				if ( $hs eq 4 ) {
					...
					print  OUTFILE "		Reply $srcip -> $desname timed out\n";
					#print Dumper( \%hohoh );
			...
			print  "Origin $srcname \t Total  ",$sessions{ $srcip }{o_T_bytes}, " / ", $sessions{ $srcip }{r_T_bytes};
