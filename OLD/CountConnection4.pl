...
while (1) {
	...
	$timestamp_old = $timestamp;
		...
		print  " --------- $timestamp ", scalar localtime( $timestamp ), "----ts $t1 ts_old $t2-----\n";
		print  OUTFILE " --------- $timestamp ", scalar localtime( $timestamp ), "----ts $t1 ts_old $t2-----\n";
		...
		foreach my $srcip ( sort keys %sessions )  {
			...
			foreach my $desip (sort keys %hoh ) {
				...
				#print " $srcip connected to $desip getting deltas\n";
				...
				foreach my $ports (sort keys %hohoh ) {
					...
					if ($t ne $timestamp ){
						print  OUTFILE "		Session Expired $desip $ports TOTAL byte previously reported $sb ", " / "," $db \n";
						...
						#print "		session $ports active delta reporting ",$sessions{ $srcip }{ $desip }{o_T_bytes}, " / ", $sessions{ $srcip }{ $desip }{r_T_bytes}, "\n";
					}
				}
				...
				print  OUTFILE "	Reply $desip \t Total  ",$sessions{ $srcip }{ $desip }{o_T_bytes}, " / ", $sessions{ $srcip }{ $desip }{r_T_bytes};
				...
				if ( $hs eq 4 ) {
					...
					print  OUTFILE "		Reply $srcip -> $desip timed out\n";
					#print Dumper( \%hohoh );
			...
			print  "Origin $srcip \t Total  ",$sessions{ $srcip }{o_T_bytes}, " / ", $sessions{ $srcip }{r_T_bytes};
